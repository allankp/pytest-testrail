...
def pytest_addoption(parser):
    ...
    group.addoption(
        ...
        help='Path to the config file containing information about the '
             'TestRail server (defaults to testrail.cfg)')
    group.addoption(
        '--tr-url',
        help='TestRail address you use to access TestRail with your web '
             'browser (config file: url in API section)')
    group.addoption(
        '--tr-email',
        help='Email for the account on the TestRail server (config file: '
             'email in API section)')
    group.addoption(
        '--tr-password',
        help='Password for the account on the TestRail server (config file: '
             'password in API section)')
    group.addoption(
        '--tr-testrun-assignedto-id',
        help='ID of the user assigned to the test run (config file: '
             'assignedto_id in TESTRUN section)')
    group.addoption(
        '--tr-testrun-project-id',
        help='ID of the project the test run is in (config file: project_id '
             'in TESTRUN section)')
    group.addoption(
        '--tr-testrun-suite-id',
        help='ID of the test suite containing the test cases (config file: '
             'suite_id in TESTRUN section)')
    group.addoption(
        '--tr-testrun-suite-include-all',
        action='store_true',
        default=None,
        help='Include all test cases in specified test suite when creating '
             'test run (config file: include_all in TESTRUN section)')
    group.addoption(
        '--tr-testrun-name',
        help='Name given to testrun, that appears in TestRail (config file: '
             'name in TESTRUN section)')
    group.addoption(
        '--tr-run-id',
        default=0,
        required=False,
    ...
    group.addoption(
        ...
        default=0,
        required=False,
        help='Identifier of testplan, that appears in TestRail. If provided, '
             'option "--tr-testrun-name" will be ignored')
    group.addoption(
        '--tr-version',
        default='',
    ...
    group.addoption(
        ...
        action='store_false',
        required=False,
        help='Determine if results of "blocked" testcases (in TestRail) are '
             'published or not')
    group.addoption(
        '--tr-skip-missing',
        action='store_true',
        ...
        config.pluginmanager.register(
            PyTestRailPlugin(
                ...
                include_all=config_manager.getoption(
                    'tr-testrun-suite-include-all', 'include_all', 'TESTRUN',
                    is_bool=True, default=False),
                cert_check=config_manager.getoption(
                    'tr-no-ssl-cert-check', 'no_ssl_cert_check', 'API',
                    is_bool=True, default=True),
                tr_name=config_manager.getoption(
                    'tr-testrun-name', 'name', 'TESTRUN'),
...
class ConfigManager(object):
    def __init__(self, cfg_file_path, config):
        ...
        Config options set in flags are given preference over options set in
        the config file.

        :param cfg_file_path: Path to the config file containing information
        about the TestRail server.
    ...
    def getoption(self, flag, cfg_name, section=None, is_bool=False,
                  default=None):
        ...
        if self.cfg_file.has_option(section, cfg_name):
            ...
            return self.cfg_file.getboolean(section, cfg_name) if is_bool \
