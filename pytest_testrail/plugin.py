...
warnings.simplefilter(
    action='once', category=DeprecatedTestDecorator, lineno=0)


class pytestrail(object):
    """An alternative to the testrail function as a decorator.

    Pytest may confuse `testrail` as a test function since it has the 'test'
    prefix.
    """
...
def testrail(*ids):
    ...
    """
    deprecation_msg = (
        'pytest_testrail: the @testrail decorator is deprecated and will be '
        'removed. Please use the @pytestrail.case decorator instead.')
...
def clean_test_ids(test_ids):
    ...
    """
    return [int(re.search('[0-9]+$', test_id).group()) for test_id in test_ids]
...
class PyTestRailPlugin(object):
    def __init__(
            self, client, assign_user_id, project_id, suite_id, include_all,
            cert_check, tr_name, run_id=0, plan_id=0, version='',
            close_on_complete=False, publish_blocked=True, skip_missing=False):
    ...
    def pytest_runtest_makereport(self, item, call):
        """Collect result and associated testcases of an execution."""
        outcome = yield
    ...
    def pytest_sessionfinish(self, session, exitstatus):
        ...
        if self.results:
            ...
            print('[{}] Testcases to publish: {}'.format(
            ...
            if self.testrun_id:
                ...
                print('[{}] Testruns to update: {}'.format(
    ...
    def add_results(self, testrun_id):
        ...
        try:
            ...
            def converter(s, c):
                return str(bytes(s, "utf-8"), c)

        # Results are sorted by 'case_id' and by 'status_id' (worst result at
        # the end)
        ...
        if self.publish_blocked is False:
            print(
                '[{}] Option "Don\'t publish blocked testcases" '
                'activated'.format(TESTRAIL_PREFIX))
            blocked_tests_list = [
                test.get('case_id') for test in self.get_tests(testrun_id)
                if test.get('status_id') == TESTRAIL_TEST_STATUS["blocked"]
            ]
            print('[{}] Blocked testcases excluded: {}'.format(
                TESTRAIL_PREFIX,
        ...
        # prompt enabling include all test cases from test suite when
        # creating test run
        if self.include_all:
            print(
                '[{}] Option "Include all testcases from test suite for test '
                'run" activated'.format(TESTRAIL_PREFIX))
        ...
        for result in self.results:
            ...
            if comment:
                # Indent text to avoid string formatting by TestRail. Limit
                # size of comment.
                data['comment'] = u"# Pytest result: #\n"
                data['comment'] += u'Log truncated\n...\n' \
                    if len(str(comment)) > COMMENT_SIZE_LIMIT else u''
                unicode_comment = converter(str(comment), "utf-8")
                truncated_comment = unicode_comment[-COMMENT_SIZE_LIMIT:]
                data['comment'] += u"    " + \
            ...
            response = self.client.send_post(
                ADD_RESULT_URL.format(testrun_id, result['case_id']), data,
                cert_check=self.cert_check
            )
            error = self.client.get_error(response)
            if error:
                print(
                    '[{}] Info: Testcase #{} not published for following '
                    'reason: "{}"'.format(
                        TESTRAIL_PREFIX, result['case_id'], error))

    def create_test_run(
            self, assign_user_id, project_id, suite_id, include_all,
            testrun_name, tr_keys):
        ...
        response = self.client.send_post(
            ADD_TESTRUN_URL.format(project_id), data,
            cert_check=self.cert_check)
        error = self.client.get_error(response)
        if error:
            print('[{}] Failed to create testrun: "{}"'.format(
                TESTRAIL_PREFIX, error))
        else:
            self.testrun_id = response['id']
            print('[{}] New testrun created with name "{}" and ID={}'.format(
                TESTRAIL_PREFIX, testrun_name, self.testrun_id))

    def close_test_run(self, testrun_id):
        """Closes testrun."""
        response = self.client.send_post(
            CLOSE_TESTRUN_URL.format(testrun_id), data={},
            cert_check=self.cert_check)
        error = self.client.get_error(response)
        if error:
            print('[{}] Failed to close test run: "{}"'.format(
                TESTRAIL_PREFIX, error))
        else:
            print('[{}] Test run with ID={} was closed'.format(
                TESTRAIL_PREFIX, self.testrun_id))

    def close_test_plan(self, testplan_id):
        """Closes testplan."""
        response = self.client.send_post(
            CLOSE_TESTPLAN_URL.format(testplan_id), data={},
            cert_check=self.cert_check)
        error = self.client.get_error(response)
        if error:
            print('[{}] Failed to close test plan: "{}"'.format(
                TESTRAIL_PREFIX, error))
        else:
            print('[{}] Test plan with ID={} was closed'.format(
    ...
    def is_testrun_available(self):
        ...
        error = self.client.get_error(response)
        if error:
            print('[{}] Failed to retrieve testrun: "{}"'.format(
                TESTRAIL_PREFIX, error))
            return False
    ...
    def is_testplan_available(self):
        ...
        error = self.client.get_error(response)
        if error:
            print('[{}] Failed to retrieve testplan: "{}"'.format(
                TESTRAIL_PREFIX, error))
            return False
    ...
    def get_available_testruns(self, plan_id):
        """Get the available testruns for a plan.

        :return: a list of available testruns associated to a testplan.
        """
        testruns_list = []
        response = self.client.send_get(
    ...
    def get_tests(self, run_id):
        """Get the tests contained in a run.
        ...
        error = self.client.get_error(response)
        if error:
            print('[{}] Failed to get tests: "{}"'.format(
                TESTRAIL_PREFIX, error))
            return None
        return response
