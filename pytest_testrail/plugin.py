...
class PyTestRailPlugin(object):
    ...
    def add_plan_entry(
        ...
        :param test_plan_id: id of the test plan that a run will be added to
        :param assign_user_id: id that the new test run will be assigned to
        :param project_id: project where to find the test plan
        :param suite_id: suite id - first suite id will be found and used if None passed in
        :param include_all: boolean for including all test cases in the new test run, default
        :param testrun_name: name to be used if autogenerated test run name not used
        :param tr_keys: collected testrail ids
        """
        if not suite_id:
        ...
        response = self.client.send_post(
            ADD_PLAN_ENTRY_URL.format(test_plan_id),
            data,
            cert_check=self.cert_check
        )
        error = self.client.get_error(response)
